...
class MockWillie(object):
    def __init__(self, nick, admin=False, owner=False):
        ...
        self.memory = lpbot.tools.lpbotMemory()

        self.ops = {}
        self.halfplus = {}
        self.voices = {}

        self.config = lpbot.config.Config('', load=False)
        self._init_config()
    ...
    def test():
        ...
        if hasattr(tested_func, "commands"):
            for command in tested_func.commands:
                regexp = lpbot.tools.get_command_regexp(".", command)
        ...
        pretrigger = lpbot.trigger.PreTrigger(bot.nick, full_message)
        trigger = lpbot.trigger.Trigger(bot.config, pretrigger, match)
